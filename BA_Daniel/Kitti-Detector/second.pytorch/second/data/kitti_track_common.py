...
def get_kitti_info_path(idx,
                        ...
                        relative_path=True,
                        exist_check=True):
    if info_type is 'image_02' or info_type is 'velodyne':
    ...
    file_path = pathlib.Path(info_type) / img_idx_str
...
def get_kitti_image_info(path,
    ...
    if image_ids is None:
        img_path = path+'image_02/'
        image_ids = list(range(len(glob.glob(img_path+'*.png'))))
        print('Found '+str(len(image_ids))+'  Images at '+str(img_path))
