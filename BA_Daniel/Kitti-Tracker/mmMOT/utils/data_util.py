...
#       calib[seq_id] = read_calib_file(f"{root_dir}/calib/{seq_id}.txt")
    ...
    with open(f"{root_dir}/oxts/oxts.txt") as f_oxts:
        oxts_seq = f_oxts.readlines()
    calib = read_calib_file(f"{root_dir}/calib/calib.txt")


#   lines is line of text file xxxx-yyyyyyy
    ...
    for line in lines:
        ...
        for x in detections:
            ...
            if len(x['name']) == 0:
                ...
                has_det = True
                break
        pos, rad = get_pos(oxts_seq, int(img_frame_id))
