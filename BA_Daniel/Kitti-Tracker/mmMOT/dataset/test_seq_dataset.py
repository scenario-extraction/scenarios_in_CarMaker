...
#	with open(link_file) as f:
        ...
        self.seq_ids = ['0000'] #added by dajo

        #for line in lines:
         #   id_path = line.strip()
          #  img_seq_id = id_path.split('-')[0]
           # img_frame_id = id_path.split('-')[1]
            #self.seq_ids.append(img_frame_id)
...
class TestSequence(Dataset):
    ...
    def _generate_img(self, idx):
        ...
        for frame in frames:
            frame['image_path'] = frame['frame_id']+".png" #added because of new folder structure
    ...
    def _generate_img_lidar(self, idx):
        ...
        for frame in frames:
            frame['image_path'] = frame['frame_id']+".png"#added because of new folder structure
            path =f"{self.root_dir}/image_02/{frame['image_path']}" 
            #path =f"{self.root_dir}/image_02/{frame['frame_id']}.png"#for mmmot support
            img = Image.open(path)
            det_num = frame['detection']['bbox'].shape[0]
