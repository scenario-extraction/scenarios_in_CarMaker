...
Vhcl_ModelCheck_BeforePre (struct tInfos *Inf)
{
	...
	Car_ModelCheck_BeforePre(Inf);
    if (!BrakeDisabled)
	Brake_ModelCheck_BeforePre(Inf);
    if (!PowerTrainDisabled)
	PowerTrain_ModelCheck_BeforePre(Inf);
    Trailer_ModelCheck_BeforePre(Inf);
    ModelCheck_End();
    return 0;
}
...
Vhcl_ModelCheck_AfterPre (struct tInfos *Inf)
{
    ...
    if (Vehicle.Model.Source == VehicleSource_BuiltIn)
	Car_ModelCheck_AfterPre(Inf);
    if (SimCore.Trailer.nTrailers != 0)
	Trailer_ModelCheck_AfterPre(Inf);
    ModelCheck_End();
    return 0;
}
