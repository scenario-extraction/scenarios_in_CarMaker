...
%% Maximaum signal value und minumum signal value in this sliding window loop
   ...
   [signal_object_value_min,j] = min(signal_object_value);
...
%                   elseif delta_signal_object_value <= th_value_label_object && delta_signal_object_value >= th_value_label_ego_same  %label(0, -1) Ego and Object take same lane change
%                          label_object = -1 ;
%                          d_object = d_object_lcl ;
...
% results = [number_loop, lstart_time, lstop_time, msm_d_ego, labeling_ego, msm_d_object, labeling_object];     
...
% title = {'Number of loop', 'Loop start time', 'Loop stop time', 'MSM_d_ego','labeling','MSM_d_ego','labeling'};
...
Output_label = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\DTW_results_highway_1_new_OT_1.xlsx',output ,'Label_lat_msm');
