%% Compare WDTW_label_data with ground truth label data
clc;
tic;
%% Load WDTW Label and Ground Truth Label
% data = load ('E:\CM_Projects\CM7_Highway\src\dtw_results_lat.mat');
data = load ('C:\Users\scuwa\Desktop\MA_Jiawei\WDTW_results_lat.mat');
...
wdtw_labeling_ego = data.labeling_ego ;
wdtw_labeling_object = data.labeling_object ;
...
for n = 2 : NumberOfLoop - 2
    ...
    if wdtw_labeling_ego(n) == wdtw_labeling_ego(n - 1) &&  wdtw_labeling_ego(n) ~= 0
        
        m = m + 1; 
               
     else
              if m >= 5
                 A = data.wdtw_d_ego(n-m: n -1);
                 [B,p] = min(A);      
                 I =[p-2;p-1;p;p+1;p+2];            
                 for j = 1 : length(A)
                     if ismember(j, I) == 1
                         wdtw_labeling_ego(n-m-1+j) = wdtw_labeling_ego(n-m-1+j);
                     else
                         wdtw_labeling_ego(n-m-1+j) = 0;
                     end                    
                 end                
              end 
        m = 1;
...
% Object results sorting
    ...
    if wdtw_labeling_object(n) == wdtw_labeling_object(n -1) &&  wdtw_labeling_object(n) ~= 0
        
        m1 = m1 + 1; 

     else
              if m1 >= 5
                A = data.wdtw_d_object(n-m1 : n -1);
                [B,p] = min(A); 
                 I =[p-2;p-1;p;p+1;p+2];                
                 for j = 1 : length(A)
                     if ismember(j, I) == 1
                         wdtw_labeling_object(n-m1-1+j) = wdtw_labeling_object(n-m1-1+j);
                     else
                         wdtw_labeling_object(n-m1-1+j) = 0;
                     end                    
                 end                
              end 
        m1 = 1; 
...
%% WDTW labeling Ego and Object
for n = 1 : NumberOfLoop - 2
 % Ego labeling resuts
    if wdtw_labeling_ego(n) == wdtw_labeling_ego(n +1)  
        start_time_ego(k) = start_time_ego(k);
        stop_time_ego(k) = lstop_time(n+1) ;
        labeling_ego(k) = wdtw_labeling_ego(n);            
    else
        k = k + 1;
        start_time_ego(k) = lstart_time(n+1);
        stop_time_ego(k) = lstop_time(n+1) ;
        labeling_ego(k) = wdtw_labeling_ego(n+1);        
    end
    
  % Object labeling results
  
    if wdtw_labeling_object(n) == wdtw_labeling_object(n +1)  
        start_time_object(k1) = start_time_object(k1);
        stop_time_object(k1) = lstop_time(n+1) ;
        labeling_object(k1) = wdtw_labeling_object(n);           
    else
        k1 = k1 + 1;
        start_time_object(k1) = lstart_time(n+1);
        stop_time_object(k1) = lstop_time(n+1) ;       
        labeling_object(k1) = wdtw_labeling_object(n+1);    
    end        
end
...
% Output_label1 = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\Accuracy_slidingwindow.xlsx',output1 ,'results_WDTW',cellnames_ego);
% Output_label2 = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\Accuracy_slidingwindow.xlsx',output2 ,'results_WDTW',cellnames_object);
% Output_label3 = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\Accuracy_slidingwindow.xlsx',output3 ,'results_WDTW',cellnames_gt_ego);
% Output_label4 = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\Accuracy_slidingwindow.xlsx',output4 ,'results_WDTW',cellnames_gt_object);
% Output_label5 = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\Accuracy_slidingwindow.xlsx',output5 ,'results_WDTW',cellnames_output);
%%
toc;
save output.mat;