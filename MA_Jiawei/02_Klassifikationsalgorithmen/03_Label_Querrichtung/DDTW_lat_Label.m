%%  Label_object_lat with DDTW and slidingwindow (lcr = lane change right, lcl = lane change left, lk = lane keeping)
...
%% Choose Signal for DDTW Process(Orignal siganl and reference signal)
...
%% Maximaum signal value und minumum signal value in this sliding window loop
 ...
 %% DDTW algorithm - Ego lane change left   
    % Input signal
    X = S_ego';
    Y = R_ego_lcl;
    N = size(X,1);
    ...
    % Time series derivative processed   
    for i = 2 : N - 1
        X(i) = 1/2 *(X(i)-X(i-1)) + 1/4 * (X(i+1)-X(i-1));
    end
    for i = 2 : M - 1
        Y(i) = 1/2 *(Y(i)-Y(i-1))+ 1/4 * (Y(i+1)-Y(i-1));
    end
  ...
  %% DDTW algorithm - Ego lane change right   
     % Input signal
    X = S_ego';
    Y = R_ego_lcr;
    N = size(X,1);
    ...
    %% DDTW algorithm - Object lane change left   
          % Input signal
    X = S_object';
    Y = R_object_lcl;
    N = size(X,1);
  ...
  %% DDTW algorithm - Object lane change right   
     % Input signal
    X = S_object';
    Y = R_object_lcr;
    N = size(X,1);
...
if d_object_lcl <= th_value_lcl_object                 % ddtw results shows lane change left identity (maybe object lane change or ego lane change)
...
elseif d_object_lcr <= th_value_lcr_object                % ddtw results shows lane change right identity (maybe object lane change or ego lane change or together lane change)
 ...
 %% Output(number of Loop,Sliding window start/stop time, DDTW distance, labeling)  
  
   number_loop(k,1) = nrOfLoop;
   lstart_time(k,1) = loop_start_time;
   lstop_time(k,1) = loop_stop_time;
   ddtw_d_object(k,1) = d_object;
   ddtw_d_ego(k,1) = d_ego;
...
%% Results output
% results = [number_loop, lstart_time, lstop_time, all_signal_ego_value_max, all_signal_ego_value_max_time, all_delta_signal_ego_value, all_d_ego_lcl,all_d_ego_lcr, ddtw_d_ego, labeling_ego, all_signal_object_value_max, all_signal_object_value_max_time, all_delta_signal_object_value, all_d_object_lcl,all_d_object_lcr, ddtw_d_object,labeling_object];    
% results_cell = num2cell(results);  
% title = {'Number of loop', 'Loop start time', 'Loop stop time', 'Maximal Ego signal value in loop', 'Maximal Ego signal value time' , 'Delta Ego signal value', 'Real DDTW Ego Dis.with Rsignal_lcl','Real DDTW Ego Dis.with Rsignal_lcr', 'DDTW Ego Distance', 'Labeling Ego','Maximal Object signal value in loop', 'Maximal Obeject signal value time' , 'Delta Object signal value', 'Real DDTW Object Dis.with Rsignal_lcl', 'Real DDTW Object Dis.with Rsignal_lcr','DDTW Object Distance', 'Labeling Object' };
% output = [title ; results_cell];  
% Output_label = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\DDTW_results_highway_T10.xlsx',output ,'Label_lat_ddtw');

toc;
Runtime = toc;
save ('ddtw_results_lat.mat');
