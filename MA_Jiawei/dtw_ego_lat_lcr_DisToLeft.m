...
windLen = 0.25 * N_lcr ;            % Window length
...
th_value_lcr = 52;                 % (14)Threshold value for lane change right classification
% th_value_lcl = 3;                   % Threshold value for lane change left classification
...
%%  Labeling (lane change right, lane chang left, lane keeping)     
    ...
    if d_lcr <= th_value_lcr 
        ...
        if signal_value_max > 1           % 1 meter is relevant with lane width
          label = 1 ;                    % 1 means lane change right happens
          d = d_lcr ; 
           
          dtw_results.lcr.label(t) = label;
          dtw_results.lcr.start_time(t) = loop_start_time;
          dtw_results.lcr.stop_time(t) = loop_stop_time;
          t = t + 1;
           
        elseif  signal_value_max > 0.5
                label = 2;                    % 2 means attempt lane change right
                d = 0;
        end
     else
         label = 3;                     % 3 means no lane change right happens
         d = 0;
    end
...
results = [number_loop, lstart_time, lstop_time, all_signal_value_max, all_signal_value_max_time, all_d_lcr, dtw_d, labeling];          

results_cell = num2cell(results);  

title = {'Number of loop', 'Loop start time', 'Loop stop time', 'Maximal signal value in loop', 'Maximal signal value time', 'Real DTW Dis.with Rsignal_lcr ', 'DTW Distance', 'Labeling' };

output = [title ; results_cell]; 

save ('dtw_results.mat','dtw_results');

Output_label = xlswrite('C:\Users\scuwa\Desktop\MA_Jiawei\Data\DTW_results_highway_06_0.25.xlsx',output ,'Label_ego_lcr_DistToLeft');
