...
% Model 1 constants after enlarging of test set
% c_height = 4.174265319755178e-04; c_width = 4.533743889453055e-04;
load('var_optimal_no_bias_model_1');
...
camera
Concept:
y_theoretical_photo = y_from_front_camera / (c_width * x_from_front_camera)
z_theoretical_photo = z_from_front_camera / (c_height * x_from_front_camera)
%}
...
Assoc.front.theoretical_image_pos.y = Assoc.front.data.y ./ (c_width .* Assoc.front.data.x);
Assoc.front.theoretical_image_pos.z = Assoc.front.data.z ./ (c_height .* Assoc.front.data.x);
...
%% Calculate theoretical positions in image [pixels]
Assoc.rear.theoretical_image_pos.y = Assoc.rear.data.y ./ (c_width .* Assoc.rear.data.x);
Assoc.rear.theoretical_image_pos.z = Assoc.rear.data.z ./ (c_height .* Assoc.rear.data.x);
...
%% Calculate real height and width of traffic objects [m]
%{
Concept:
height_real = height_image * c_height * x_from_camera
width_real = width_image * c_width * x_from_camera
%}
%% Front Camera
% Width
for bb = 1:size(bboxes_front, 1)
    if ~isnan(bboxes_front(bb, 6))
        ...
        % Width
        bboxes_front(bb, 8) = bboxes_front(bb, 3) * c_width * radar.from_front_camera.x(bboxes_front(bb, 5) .* freq, bboxes_front(bb, 6));
        % Height
        bboxes_front(bb, 9) = bboxes_front(bb, 4) * c_height * radar.from_front_camera.x(bboxes_front(bb, 5) .* freq, bboxes_front(bb, 6));
    else
...
for bb = 1:size(bboxes_rear, 1)
    if ~isnan(bboxes_rear(bb, 6))
        % If bounding box has an associated traffic object
        % Width
        bboxes_rear(bb, 8) = bboxes_rear(bb, 3) * c_width * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6)) - avg_car_length);
        % Height
        bboxes_rear(bb, 9) = bboxes_rear(bb, 4) * c_height * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6)) - avg_car_length);
    else
...
%% Treat outliers
%{
Separate categories for outliers treatment: (width, height) X Traffic objects 
%}
%% Width
...
for obj = (1:size(ids, 1))
    ...
    disp(widths_array(3, obj) - 3 * widths_array(2, obj));
    disp(widths_array(3, obj) + 3 * widths_array(2, obj));
    %}
    logical = widths_obj >= med - 3 * stDev & widths_obj <= med + 3 * stDev;
...
for obj = (1:size(ids, 1))
    ...
    disp(heights_array(3, obj) - 3 * heights_array(2, obj));
    disp(heights_array(3, obj) + 3 * heights_array(2, obj));
    %}
    logical = heights_obj >= med - 3 * stDev & heights_obj <= med + 3 * stDev;
...
if testing == 1
    ...
    %% Calculate mean absolute error for width and height
    mae.width = mean(abs(widths_array(2, :) - widths_array(3, :)));
    mae.height = mean(abs(heights_array(2, :) - heights_array(3, :)));
    
    %% Print mean absolute error to command window
    string = strcat('\n Mean absolute deviation from ground truth height and width: \n height:', 32, num2str(mae.height), 32, 'm \n width:', 32, num2str(mae.width), 32, 'm \n');
    fprintf(string);
end
...
%% Load tensor
load('C:\Users\philipp\Documents\GitHub\BA_1\src\006 Get Dimensions aktuell\Data Application\Tensoren\Tensoren.mat');
tensors = struct;
...
save('C:\Users\philipp\Documents\GitHub\BA_1\src\006 Get Dimensions aktuell\Results Application\tensors', 'tensors');


%{
%% Some plotting
% car 1
where = [1:6,8,9,11,13,14,16,18,20,22,24,26];
scatter((1:size(where, 2)), bboxes_front(where, 7));
scatter((1:size(where, 2)), bboxes_front(where, 8));
% car 2
where = [38:45,47:56,58,60,62,63,65,67,68,70,72,73,76,78:80,82:85,87:154];
scatter((1:size(where, 2)), bboxes_front(where, 7));
scatter((1:size(where, 2)), bboxes_front(where, 8));
%}
