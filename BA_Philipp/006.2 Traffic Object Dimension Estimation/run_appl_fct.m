...
Bounding boxes have to be imported by double clicking first.
...
% Run main_appl_model_2_as_fct for defined values and save metrics in
% 'result'.
index = 0;
for i = i_range
    for j = j_range
        for k = k_range
            ...
            if toleranceMode
                disp(strcat("std. dev. multilier for outlier treatment: ", num2str(k)));  
            end
            index = index + 1;
            result(index, 1) = i;
            result(index, 2) = j;
            [metrics] = appl_model_2_as_function(k, i, j, bboxes_front, bboxes_rear);
            if front
...
if front
    ...
    legend('MSE front width', 'MSE front height');
else
    plot(result(:,2), result(:,3), '-o', 'MarkerSize', 5);
    plot(result(:,2), result(:,4), '-o', 'MarkerSize', 5);
    legend('MSE rear width', 'MSE rear height');
end
hold off;

%{ 
Result: 
From looking at the plots it can be said that for this dataset 23 m for 
front and 17 m for rear seem to be good limits.
%}
...
%{
Result:
Looking at plots for tolerance values in [0, 5] (stride 0.1) it seems like
outlier treatment has no profitable impact. Tolarance value will be set to
a high value (for example 5) that has no effect
%}