...
rudimentarily, 4.5 m (which I believe is about the length of a standard
car) is subtracted from the distance. 
...
for bb = 1:size(bboxes_rear, 1)
    ...
    if ~isnan(bboxes_rear(bb, 6))
        if radar.from_rear_camera.x(time * freq, object) >= -(limit_rear + car_length)
            % disp(strcat('time: ', num2str(time), ', object: ', num2str(object)));
            bboxes_rear(bb, 6) = NaN;
        end
    end
...
for bb = 1:size(bboxes_rear, 1)
    if ~isnan(bboxes_rear(bb, 6))
        % If bounding box has an associated traffic object
        % Width
        bboxes_rear(bb, 8) = bboxes_rear(bb, 3) * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6)) - car_length) / c_width;
        % Height
        bboxes_rear(bb, 9) = bboxes_rear(bb, 4) * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6)) - car_length) / c_height;
    else
