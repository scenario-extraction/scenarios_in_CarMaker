...
% Model constants (hardcopy of model constants: 
...
for bb = 1:size(bboxes_rear, 1)
    if ~isnan(bboxes_rear(bb, 6))
        % If bounding box has an associated traffic object
        % Width
        bboxes_rear(bb, 8) = bboxes_rear(bb, 3) * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6))) / c_width;
        % Height
        bboxes_rear(bb, 9) = bboxes_rear(bb, 4) * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6))) / c_height;
    else
