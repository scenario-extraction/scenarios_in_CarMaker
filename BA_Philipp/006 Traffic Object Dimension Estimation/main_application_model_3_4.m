...
To make yellow bounding boxes in CarMaker invisible: In IPGMovie: View - Show - Sensors
%}
...
% Do you want to detect bounding boxes from images provided or are they already
% laoded into the workspace? Detect: 1, Already loaded: 0
detect_bboxes = 1;
...
% Do you want to save (and potentially overwrite) tensors on hard drive 
% after running this script? Save: 1, Don't save: 0
saveTensor = 0;

% Main Directory
mainDir = 'C:\Users\philipp\Documents\GitHub\BA_1\src\006 Traffic Object Dimension Estimation';

% Select distance in which traffic objects are ignored (see section 
...
% Tolerance for outlier treatment. Any values outside 
% median +- tol * st. dev. 
% of respective detection object will be discarded
tol = 4;

% Model constants (hardcopies of model constant model 3 and 4:
% c = 2.188958364298405e+03;
load(strcat(mainDir, '\Results Training\var_optimal_no_bias_model_3'));
c = var.c;
...
radar.raw = struct;
for k = 0:9
    if isfield(data_radar.data, ['Sensor_Object_OB01_Obj_T0' num2str(k) '_NearPnt_ds_x']) == 1
        n = k+1;
...
for k = 10:256
    if isfield(data_radar.data, ['Sensor_Object_OB01_Obj_T' num2str(k) '_NearPnt_ds_x']) == 1
        n = k+1;
...
camera
Concept:
y_theoretical_photo = y_from_front_camera / x_from_front_camera * c
z_theoretical_photo = z_from_front_camera / x_from_front_camera * c
%}
...
Assoc.front.theoretical_image_pos.y = Assoc.front.data.y ./ Assoc.front.data.x .* c;
Assoc.front.theoretical_image_pos.z = Assoc.front.data.z ./ Assoc.front.data.x .* c;
...
%% Calculate theoretical positions in image [pixels]
Assoc.rear.theoretical_image_pos.y = Assoc.rear.data.y ./ Assoc.rear.data.x .* c;
Assoc.rear.theoretical_image_pos.z = Assoc.rear.data.z ./ Assoc.rear.data.x .* c;
...
for bb = 1:size(bboxes_rear, 1)
    ...
    if ~isnan(bboxes_rear(bb, 6))
        if radar.from_rear_camera.x(time * freq, object) >= -(limit_rear)
            if dispDiscard
                disp(strcat('time: ', num2str(time), ', object: ', num2str(object)));
            end
            bboxes_rear(bb, 6) = NaN;
...
%{
Concept:
height_real = height_image * x_from_camera / c
width_real = width_image * x_from_camera / c
%}
%% Front Camera
% Width
for bb = 1:size(bboxes_front, 1)
    if ~isnan(bboxes_front(bb, 6))
        ...
        % Width
        bboxes_front(bb, 8) = bboxes_front(bb, 3) * radar.from_front_camera.x(bboxes_front(bb, 5) .* freq, bboxes_front(bb, 6)) / c;
        % Height
        bboxes_front(bb, 9) = bboxes_front(bb, 4) * radar.from_front_camera.x(bboxes_front(bb, 5) .* freq, bboxes_front(bb, 6)) / c;
    else
...
for bb = 1:size(bboxes_rear, 1)
    if ~isnan(bboxes_rear(bb, 6))
        % If bounding box has an associated traffic object
        % Width
        bboxes_rear(bb, 8) = bboxes_rear(bb, 3) * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6))) / c;
        % Height
        bboxes_rear(bb, 9) = bboxes_rear(bb, 4) * (-radar.from_rear_camera.x(bboxes_rear(bb, 5) .* freq, bboxes_rear(bb, 6))) / c;
    else
...
%% Load tensor
load(strcat(mainDir, '\Data Application\tensors\Tensoren.mat'));
tensors = struct;
