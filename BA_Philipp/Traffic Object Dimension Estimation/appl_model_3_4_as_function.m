function [metrics] = appl_model_3_4_as_function(tol, limit_front, limit_rear, bboxes_front, bboxes_rear)

%{
This function is to be called from script 'run_appl_fct' in this folder and
is equivalent to 'main_application_model_3_4'

The purpose of this being a function is to try many different parameters.

The output is a struct containing different sums of squares and related 
numbers of observations to compute mean square error (MSE) and two different 
mean absoute error (MAE) results for assessing the the model's quality.
%}
    ...
    % *************************************************************************
    %
    % This script is for applying model 3 or 4 trained with "main_training"
    %
    ...
    %% Set model constants 
    % (hardcopy of model constant: c = 2.281898679371111e+03)
    ...
    for bb = 1:size(bboxes_front, 1)
        if ~isnan(bboxes_front(bb, 6))
            ...
            % Width
            bboxes_front(bb, 8) = bboxes_front(bb, 3) * radar.from_front_camera.x(int64(bboxes_front(bb, 5) .* freq), bboxes_front(bb, 6)) / c;
            % Height
            bboxes_front(bb, 9) = bboxes_front(bb, 4) * radar.from_front_camera.x(int64(bboxes_front(bb, 5) .* freq), bboxes_front(bb, 6)) / c;
        else
    ...
    for bb = 1:size(bboxes_rear, 1)
        if ~isnan(bboxes_rear(bb, 6))
            % If bounding box has an associated traffic object
            % Width
            bboxes_rear(bb, 8) = bboxes_rear(bb, 3) * (-radar.from_rear_camera.x(int64(bboxes_rear(bb, 5) .* freq), bboxes_rear(bb, 6))) / c;
            % Height
            bboxes_rear(bb, 9) = bboxes_rear(bb, 4) * (-radar.from_rear_camera.x(int64(bboxes_rear(bb, 5) .* freq), bboxes_rear(bb, 6))) / c;
        else
